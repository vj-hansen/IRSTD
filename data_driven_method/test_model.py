...
from dd_utils import get_iou, read_xml, save_image
...
def detFP(
        gtObjInImg, readXmlFile, model,
        scoreThresh, imgFilename, imgs) -> int:
    FALSE_POS += 1
    # FP: predicts that an object exist when it DNE
    for i in range(gtObjInImg):
        top_left_crner = (readXmlFile[i][1], readXmlFile[i][2])
        btm_right_crner = (readXmlFile[i][3], readXmlFile[i][4])
        save_image(
            imgFilename[it], imgs[it], "fp", model,
            scoreThresh, top_left_crner, btm_right_crner)
    return FALSE_POS
...
def main():
    ...
    for it, file in enumerate(filelist):
        ...
        if (number_of_boxes-GT_OBJECTS_IN_IMG > 0) and (GT_OBJECTS_IN_IMG != 0):
            detFP(
                GT_OBJECTS_IN_IMG, read_xml_file, MODEL,
                SCORE_THRESH, img_filename, images)
