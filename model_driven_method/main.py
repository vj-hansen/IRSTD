...
for it, file in enumerate(filelist):
    ...
    im_shape = (m, n)
    start = time.time()
    T = pcp_func( img, im_shape, max_iter=MAX_IT_PARAM, tol=TOL_PARAM,
                  method=METHOD_PARAM, sw_step_size=SLIDEWIN_STEP_SIZE,
    ...
    circ_img_rgb, pcx_pos, pcy_pos = get_target_loc('t_img.jpg', thresh = THRESH_PARAM, delta=DELTA)
